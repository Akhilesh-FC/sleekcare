...
$sql = "SELECT medication.*, medication.date AS dateeeeee, patients.name AS patientsname, patients.phone AS patientsphone, patients.uhid,docters.name AS doctor_name, docters.logo, docters.signature, docters.about,docters.mobile, docters.time AS date,docters.hospital_number AS hospital_name, docters.address,medication.blood_group AS blood_pressure, medication.tables AS after_food , medication.notes_remark AS notes_remarks ,medication.medical_details ,medication.date AS date_time, docters.time AS date ,docters.address FROM medication JOIN appointments ON medication.appointment_id = appointments.id JOIN patients ON appointments.phone = patients.phone JOIN docters ON docters.id=medication.doctor_id WHERE patients.phone ='$phone' AND medication.randomnumber='$date_t'";
