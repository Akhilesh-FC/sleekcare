...
$sql = "SELECT appointments.name, appointments.age, appointments.gender, appointments.uhid, appointments.phone, 
               medication.id, docters.name AS doctor_name,medication.date AS dateeee, docters.logo, medication.appointment_id AS App_id,
               docters.signature, docters.about, medication.notes_remark AS notes_remarks, medication.medicine_remark ,medication.lab_remark ,medication.symptoms_remark ,medication.diagnosis_remark,
               medication.notes AS notes, medication.dibitic,  medication.date_time, 
               medication.spo2, medication.lab AS report_suggestion,medication.tme_to_take AS tme_to_take,medication.to_be_taken AS to_be_taken, 
               docters.hospital_number AS hospital_name, docters.address, 
               medication.temprature, medication.weights, medication.pulse, 
               medication.height, medication.tables AS after_food, 
               medication.medical_details AS medication, docters.mobile, 
               docters.time AS date, medication.blood_group AS blood_pressure ,medication.blood_pres AS blood_pres ,medication.dignosis AS dignosis	 ,medication.symptoms AS symptoms ,medication.lab AS lab
        FROM medication 
        INNER JOIN appointments ON appointments.id = medication.appointment_id 
        INNER JOIN docters ON docters.id = medication.doctor_id 
        WHERE medication.doctor_id = '$doctorid' 
          AND medication.appointment_id = '$appointment_id' 
          AND appointments.uhid = '$uhid'ORDER BY id DESC LIMIt 1;";
...
<div class="container card mt-5 p-4">
    <div class="row">
      ...
      <div class="col-md-12 col-sm-12 mt-2">
            ...
            <br>
                 <strong><b>Date :</b> </strong><span><?php echo htmlspecialchars($row["dateeee"]) ?></span>
