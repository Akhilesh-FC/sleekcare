          ...
          <table class="table align-items-center table-flush table-hover mb-5" id="example" >
            ...
            <tbody id="myTable" class="text-white">
             ...
             <tr>
                ...
                <td> <a href="doctors-appointment?id={{$item->id}}" type="button" class="btn btn-info">{{$item->total_appointments}}</a></td>
                <td> <a href="{{ route('doctors.patient', ['id' => $item->id]) }}" type="button" class="btn btn-warning">{{$item->total_patients}}</a></td>
                 <td>
                 
                    @if($item->status=='1')
                    <a href="{{route('doctor.active',$item->id)}}" class="btn btn-danger  btn-sm" data-toggle="modal" data-target="#exampleModa{{$item->id}}"><i class="fa fa-times"></i></a>
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a  href="{{route('doctor.active',$item->id)}}" type="button" class="btn btn-success">ok</a>
                    ...
                    <a href="{{route('doctor.inactive',$item->id)}}" class="btn btn-danger  btn-sm" data-toggle="modal" data-target="#exampleMod1{{$item->id}}"><i class="fa fa-times"></i></a>
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <a  href="{{route('doctor.inactive',$item->id)}}" type="button" class="btn btn-success">ok</a>
                  ...
                  <td>  <a href="{{route('doctor.edit',$item->id)}}" class="btn btn-info  btn-sm m-1"><i class="fa fa-pencil"></i></a></td>
             </tr>
             @endforeach
            </tbody>
          </table>
